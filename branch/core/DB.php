...
class Core_DB {
    ...
    public function db_connect( $db_config ,$persistent = false){
        ...
        $db_config['compress'] = isset($db_config['compress']) && $db_config['compress'] ? true : false;
        ...
        if ( $this->conn_id->real_connect($db_config['hostname'], $db_config['username'], $db_config['password'], $db_config['database'], $db_config['port'], null, $db_config['compress'])){
            ...
            if( $this->conn_id && $this->conn_id->set_charset("utf8")){
                return $this->conn_id;
            }
        }
    ...
    public function select_all( $tablename, $columns = '*', $where = [],$index_key = null ,$is_lock = false){
        $this->result_id = $this->query($this->select_context($tablename, $columns, $where,null,$is_lock));
    ...
    public function select_row(  $tablename, $columns = '*', $where = [] ,$is_lock = false){
        $where = array_merge($where,['LIMIT' => 1]);
        if ( $result_array = $this->select_all( $tablename, $columns, $where,null,$is_lock) ){
    ...
    public function insert( $tablename, $data ,$ignore = false){
        ...
        if( $this->query('INSERT '.($ignore == true ? 'IGNORE' : '').' INTO ' . $this->table_quote( $tablename ) . ' (' . implode(', ', $columns) . ') VALUES (' . implode($values, ', ') . ')') ){
    ...
    //解析 select sql
    protected function select_context($table, &$columns = null, $where = null, $column_fn = null,$is_lock = false){
        ...
        return 'SELECT ' . $column . ' FROM ' . $table_query . $this->where_clause($where).$select_for_update;
    }
    ...
    protected function array_quote($array)
    {
        ...
        {
            $temp[] = is_int($value) ? $value : $this->quote($value,true);
        }
