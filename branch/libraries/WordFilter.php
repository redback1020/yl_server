...
class Library_WordFilter
{
    private $dict = array();

    public function __construct()
    {
        $this->initDict(ROOTPATH.'config/badwords.txt');
    }

    protected function initDict( $dictPath )
    {
        $handle = fopen($dictPath, 'r');
        if (!$handle) {
            throw new ErrorException('open dictionary file error.');
        }
